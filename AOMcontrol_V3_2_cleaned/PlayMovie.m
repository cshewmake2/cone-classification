...
if SYSPARAMS.sysmode == 2 || SYSPARAMS.sysmode == 3
    if ~isempty(Mov.seq) && ischar(Mov.seq)
        ...
        while (any(temp)) %Sequence
            ...
            Mov.aom2pow(Mov.duration) = str2double(chopped(commapos(9)+1:commapos(10)-1));
            Mov.aom2offx(Mov.duration) = str2num(chopped(commapos(10)+1:commapos(11)-1));
            Mov.aom2offy(Mov.duration) = str2num(chopped(commapos(11)+1:commapos(12)-1));
            pos = 12;
